...
single_channel_coherence = function(input_path_1, input_path_2){
  
  d1 = data.frame(read_tsv(input_path_1, col_types = cols()))
  d2 = data.frame(read_tsv(input_path_2, col_types = cols()))
  ...
  tryCatch(
    #try to do this. It may fail due to "arima errors" (likely bad data)
    {
      
      analysis = wtc(
        d1,
        d2,
        nrands = 0, # If you want to do significance testing at the level of the individual
        # pair and visit, you can use Monte Carlo randomizations by specifying nrands to a positive integer, default 300.
        # This significantly increases
        # the processing time, and the risk that something crashes!
        quiet = TRUE
      )
...
analyse_visit = function(pair, visit_path, channel_names){
  results_path = paste(visit_path, "SynchronyResults", sep = "/")
  create_directory(results_path)
  
  for (channel_name in channel_names){
    results_filename_root = paste("file_" , as.character(pair) , "_" , as.character(channel_name), sep = "")
    
    channel_path_1 = paste(visit_path, '/' , '1_' , channel_name , '.tsv', sep = "")
    channel_path_2 = paste(visit_path,  '/' , '2_' , channel_name , '.tsv', sep = "")
...
analyse_pair = function(pair, channel_names){
  tryCatch(
    #try to find correct pairs
    {
      
      pair_visit1_path = paste("HaemoglobinTimeSeries/", pair , "/Visit1", sep = "")
      analyse_visit(pair, pair_visit1_path, channel_names)
      pair_visit4_path = paste("HaemoglobinTimeSeries/", pair , "/Visit4", sep = "")
      analyse_visit(pair, pair_visit4_path, channel_names)
...
# THIS TAKES A VERY LONG TIME, AND REQUIRES ~200 GB OF SPACE

for (pair in 1:23){
  analyse_pair(pair, channel_names = CHANNEL_NAMES)
